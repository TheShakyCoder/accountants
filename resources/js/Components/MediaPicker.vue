...
<template>
    ...
    <div>
        <!-- Preview -->
        <div v-if="url" class="relative rounded-xl overflow-hidden aspect-video bg-warm-100 mb-3">
        ...
        <button type="button" @click="openPicker"
                class="w-full flex flex-col items-center gap-2 p-4 border-2 border-dashed border-warm-200 rounded-xl
                       cursor-pointer hover:border-brand-300 hover:bg-brand-50/50 transition-colors">
            ...
            <span class="text-xs text-warm-500">{{ url ? 'Change image' : 'Select from media library' }}</span>
        </button>
    </div>
                ...
                <div class="bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[85vh] flex flex-col overflow-hidden">
                    ...
                    <div class="flex items-center justify-between px-6 py-4 border-b border-warm-100">
                        ...
                        <div class="flex items-center gap-3">
                            <!-- Upload button -->
                            <label class="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium bg-brand-600 text-white
                                          rounded-lg hover:bg-brand-700 transition-colors cursor-pointer">
                                <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12"/>
                                </svg>
                                {{ uploading ? 'Uploading…' : 'Upload' }}
                                <input type="file" multiple accept="image/*" class="sr-only" @change="onUpload" :disabled="uploading" />
                            </label>
                            <!-- Search -->
                            <input v-model="search" @input="onSearch" type="search" placeholder="Search…"
                                   class="px-3 py-1.5 text-xs border border-warm-200 rounded-lg bg-white text-warm-900
                                          focus:outline-none focus:ring-2 focus:ring-brand-400 focus:border-transparent transition w-40" />
                            <!-- Close -->
                    ...
                    <div class="flex-1 overflow-y-auto p-6">
                        <div v-if="loading && !media.length" class="grid grid-cols-4 sm:grid-cols-6 gap-3">
                            <div v-for="n in 12" :key="n" class="aspect-square rounded-xl bg-warm-100 animate-pulse"></div>
                        </div>

                        <div v-else-if="!media.length" class="flex flex-col items-center justify-center py-16 text-center">
                            <svg class="w-10 h-10 text-warm-300 mb-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
                                      d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"/>
                            </svg>
                        ...
                        <div v-else class="grid grid-cols-4 sm:grid-cols-6 gap-3">
                            <button v-for="item in media" :key="item.id"
                                    type="button"
                                    @click="choose(item)"
                                ...
                                <div v-else class="w-full h-full bg-warm-50 flex items-center justify-center">
                                    <svg class="w-6 h-6 text-warm-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
                                ...
                                <!-- Hover filename -->
                                <div class="absolute inset-x-0 bottom-0 bg-black/50 px-1.5 py-1 translate-y-full group-hover:translate-y-0 transition-transform">
